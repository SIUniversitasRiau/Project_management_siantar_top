<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

/*
 * @package		Controller - CIMasterArtcak
 * @author		Felix - Artcak Media Digital
 * @copyright	Copyright (c) 2014
 * @link		http://artcak.com
 * @since		Version 1.0
 * @description
 * Contoh Tampilan administrator dashbard
 */

//dapat diganti extend dengan *contoh Admin_controller / Aplikan_controller di folder core. 
// Nama kelas harus sama dengan nama file php
class Departemen extends User_Controller {

    //1. seluruh fungsi yang tidak public, menggunakan awalan '_' , contoh: _getProperty()
    //2. di atas fungsi diberikan penjelasan proses apa yang dilakukan. dari mana ambil data, 
    //inputnya apa dan outputnya apa. contoh di fungsi index()
    //3. Penamaan fungsi harus PUNUK ONTA dengan awalan huruf kecil $ Menggunakan Bahasa Inggris
    //4. Penamaan nama fungsi maksimal 3 kata

    public function __construct() {
        parent::__construct();

        $this->load->model('mDepartemen');
    }

    /*
     * Digunakan untuk menampilkan dashboard di awal. Setiap tampilan view, harap menggunakan fungsi
     * index(). Pembentukan view terdiri atas:
     * 1. Title
     * 2. Set Partial Header
     * 3. Set Partial Right Top Menu
     * 4. Set Partial Left Menu
     * 5. Body
     * 6. Data-data tambahan yang diperlukan
     * Jika tidak di-set, maka otomatis sesuai dengan default
     */

    public function index() {
        // Dua hal ini wajib di-code agar keluar data title dan memanggil view
        $this->title = "Master Departemen";
        
        $this->data['departemenList'] = $this->mDepartemen->_select(array(mDepartemen::ACTIVE => 1));
        $this->display('acDepartement', $this->data);
    }

    public function save() {
        $id = $this->input->post(mDepartemen::ID);
        if (intval($id) == 0) { // about to insert a new record
            $column = array(mDepartemen::NAME => $this->input->post(mDepartemen::NAME), mDepartemen::ACTIVE => 1);
        } else {
            $column = array(mDepartemen::ID => $id, mDepartemen::NAME => $this->input->post(mDepartemen::NAME));
        }
        //print_r($column);
        $this->mDepartemen->_insertOrUpdate($column);

        redirect(base_url() . 'user-management/departemen');
    }

    public function delete() {
        $id = $this->input->post(mDepartemen::ID);
        //print_r($id);
        $this->mDepartemen->_delete($id, mDepartemen::ACTIVE);

        redirect(base_url() . 'user-management/departemen');
    }

}
