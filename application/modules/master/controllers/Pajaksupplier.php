...
class Pajaksupplier extends Plan_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('mPajaksupplier');
    }
    ...
    public function index() {
        ...
        $this->title = "Master Pajaksupplier";
        $this->data['listPajaksupplier'] = $this->mPajaksupplier->_select(array(mPajakSupplier::ACTIVE => 1));
        $this->display('acPajaksupplier', $this->data);
    }

    public function delete() {
        $this->mPajaksupplier->_delete(array(mPajaksupplier::ID => $this->input->post("tobe_deleted_id")), mPajaksupplier::ACTIVE);
        redirect(base_url() . "master/pajaksupplier");
    }

    public function update() {
        $id = $this->input->post("PAJAK_SUPPLIER_ID");
        $input= $this->input->post();
        $this->mPajaksupplier->_update($input, array(mPajaksupplier::ID => $id));
        redirect(base_url() . "master/pajaksupplier");
    }

    public function insert() {
        $input= $this->input->post();
        $this->mPajaksupplier->_insert($input);
        redirect(base_url() . "master/pajaksupplier");
    }
